...
def split_pdf_to_png(pdf_path: str | Path, output_dir: str | Path, dpi: int = 300) -> list[Path]:
    """
    Split a PDF into one PNG image per page.

    Args:
        pdf_path: Path to the source PDF
        output_dir: Directory to write per-page PNGs into
        dpi: Render resolution (higher = sharper images)

    Returns:
        List of written PNG page paths
    """
    ...
    for i, page in enumerate(pages, start=1):
        out_path = output_dir / f"{pdf_path.stem}_page_{i:02d}.png"
...
def embed_png_images(
    ...
    """
    Load or compute embeddings for PNG images using ColPali model.

    Either loads pre-computed embeddings from a parquet file or computes them by
    processing PNG images from a directory. Supports MPS (Apple Silicon), CUDA, or CPU devices.

    Args:
        png_dir: Directory containing PNG image files
        model: Pre-loaded ColPali model (optional, will load if None)
        processor: Pre-loaded ColPaliProcessor (optional, will load if None)
        model_name: HuggingFace model name to load if model/processor not provided
        device: Device to use ('mps', 'cuda', 'cpu', or None for auto-detection)
        batch_size: Number of images to process per batch
        save_path: Path to save/load embeddings as parquet file (required for caching)
        load_precomputed: If True, load from save_path if it exists; if False, always recompute

    Returns:
        pandas DataFrame with columns: 'file_path' and 'image_embedding' (as lists)

    Example:
        >>> from colpali_engine.models import ColPali, ColPaliProcessor
        >>> import torch
        >>> model = ColPali.from_pretrained("vidore/colpali-v1.3", torch_dtype=torch.bfloat16).to("mps")
        >>> processor = ColPaliProcessor.from_pretrained("vidore/colpali-v1.3")
        >>> # First run: compute and save
        >>> df = embed_png_images(
        ...     png_dir="./rulebooks/Azul_230802_rules_pages",
        ...     model=model,
        ...     processor=processor,
        ...     device="mps",
        ...     save_path="./azul_embeddings.parquet",
        ...     load_precomputed=False
        ... )
    ...
    if load_precomputed and save_path and save_path.exists():
        try:
            df = pd.read_parquet(save_path, engine="pyarrow")
            print(f"Loaded {len(df)} precomputed embeddings from {save_path}")
            return df
        except Exception as e:
            print(f"Failed to load precomputed embeddings: {e}")
            print("Computing embeddings instead...")
    ...
    image_paths = sorted(png_dir.glob("*.png"))
    
    if not image_paths:
        raise ValueError(f"No PNG files found in {png_dir}")

    print(f"Found {len(image_paths)} PNG images to embed")
    ...
    for batch_paths in tqdm(
        ...
        # Process batch
        batch_images = processor.process_images(batch_images_list).to(device)
        with torch.no_grad():
            batch_embeddings = model(**batch_images)
    ...
    # Create DataFrame with 'file_path' column to match usage pattern
    ...
    if save_path:
        ...
        print(f"Saved {len(df)} embeddings to {save_path}")
